...
class Timer extends Anticheat implements Listener {
	...
	public function handlePacket(DataPacket|Packet $packet, NetworkSession $session) : void{
		...
		if(($player = $session->getPlayer()) === null) return;

		if(!isset($this->time[$player->getUniqueId()->__toString()])) $this->time[$player->getUniqueId()->__toString()] = 0;
		if(!isset($this->ticks[$player->getUniqueId()->__toString()])) $this->ticks[$player->getUniqueId()->__toString()] = 0;
		if(!isset($this->timer[$player->getUniqueId()->__toString()])) $this->timer[$player->getUniqueId()->__toString()] = 0;
